...
export default function BookAppointment() {
  ...
  useEffect(() => {
    if (formData.specialization && !searchParams.get("doctorId")) {
      const selectedSpec = specializations.find(
        (s) => s._id === formData.specialization
      );
      setFormData((prev) => ({
        ...prev,
        doctorId: "",
  ...
  const handleChange = (e) => {
    ...
    setFormData((prev) => ({
      ...
      ...(name === "specialization"
        ? {
            specializationName:
              specializations.find((s) => s._id === value)?.name || "",
          }
        : {}),
    }));
    setError("");
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 409) {
          ...
          setError(
            `You already have an appointment with this doctor on ${existingDate} at ${existingTime}.`
          );
        } else {
  ...
  const filteredDoctors = formData.specialization
    ? doctors.filter(
        (doc) => doc.specialization._id === formData.specialization
      )
    : [];
